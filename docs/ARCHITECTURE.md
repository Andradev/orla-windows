...
Topbars opacas e docks transparentes usam renderização por software para manter previsível a memória em GPUs integradas. Foco, disponibilidade de rede e volume usam eventos. Bateria, Bluetooth, intensidade do Wi-Fi e os estados das ações rápidas são consultados a cada 10 segundos por uma única instância compartilhada, também reutilizada pelo painel. A descoberta WMI/DDC-CI ocorre em uma thread de trabalho somente enquanto o painel está aberto e os movimentos do slider são agrupados antes da escrita. O catálogo do dock roda a cada 2 segundos. Durante o auto-hide, somente cursor e foreground são amostrados a cada 100 ms; a enumeração de sobreposição permanece em cache por 600 ms. Assim a revelação começa no primeiro tick sem transformar a enumeração de janelas em polling rápido.
...
O dock mantém o histórico dos dois últimos aplicativos externos, ignorando suas próprias janelas e janelas auxiliares do mesmo processo. Ao minimizar o aplicativo ativo, Orla envia primeiro o comando visual de minimização e só depois recupera o foco anterior, mantendo a resposta do clique imediata. Antes de ativar um app, resolve novamente seus HWNDs para não usar uma janela substituída desde o último refresh. A identidade ativa combina HWND e PID, mas exige ao menos uma janela restaurada: isso cobre diálogos e WebViews do mesmo processo sem confundir o HWND minimizado que algumas versões do Teams mantêm como foreground. Os indicadores dos dois monitores usam a mesma regra. Uma recusa temporária de foco recebe tentativas curtas e limitadas; cada tentativa relê a janela do processo, e um clique posterior invalida imediatamente a operação anterior para impedir roubo tardio de foco.

A superfície de hit-test de cada item permanece imóvel: `RenderTransform` é aplicado exclusivamente ao conteúdo. A pressão responde em 55 ms e a confirmação estabiliza em 155 ms, elevando o ícone para abrir/ativar e recuando-o para minimizar. O hover usa apenas 3,5% de escala e 1 px de deslocamento. Se `SystemParameters.ClientAreaAnimation` estiver desabilitado, todos esses movimentos são omitidos. A Lixeira vem do catálogo de ícones stock do Shell e `SHQueryRecycleBin` atualiza seu estado sem criar outro processo residente.
